    ...
    if ($_POST['action']=="add") {
        if (isset($_POST['name'])) {
            ...
            if ($query) {
                $response = array();
                $res['status_code'] = '200';
                $res['message'] = 'Success add negara.';
        ...
        if (isset($_POST['id'])) {
            $cek_query = $query = $mysqli->query("SELECT * FROM cflix_negara WHERE id='$_POST[id]' ");
            ...
            if ($cek>0) {
                if (isset($_POST['name'])) {
                    ...
                    if ($query) {
                        $response = array();
                        $res['status_code'] = '200';
                        $res['message'] = 'Success edit negara.';
                        array_push($response, $res);
                        echo json_encode($response);
                    } else {
                        $response = array();
        ...
        if (isset($_POST['id'])) {
            $query = $mysqli->query("DELETE FROM cflix_negara WHERE id = '$_POST[id]' ");
            if ($query) {
                $response = array();
                $res['status_code'] = '200';
                $res['message'] = 'Success delete negara. ' . var_dump($query);
                array_push($response, $res);
                echo json_encode($response);
            } else {
                $response = array();
        ...
        $query = $mysqli->query("SELECT * FROM cflix_negara ");
